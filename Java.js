        ...
        return this.each(function(i,v){
            var timeCountDown = {
                ...
                setHtml: function(time){
                    ...
                    //A contagem regressiva nÃ£o foi iniciada
                    if(_startLeftTime > 0){
                ...
                init: function(){
                    ...
                    if(new Date(opts.endtime) <= new Date(opts.starttime)){
                        throw new Error('A hora de inÃ¬cio da contagem regressiva nÃ£o pode ser maior que a hora final');
                        return false;
                    }
...
function vitrineOfertas(){
	if ($('.pagina-inicial #listagemProdutos a.titulo-categoria.vitrine-15962312').length){
		...
		$('<div class="textCron2">*PreÃ§os vÃ¡lidos pelo tempo da oferta ou enquanto durar nosso estoque. Aproveite!</div>'
...
$(".bandeiras-produto .bandeira-promocao").each(function() {
  	...
  	$('.preco-produto .desconto-a-vista').each(function(){
		...
		$('.preco-produto .desconto-a-vista:not(strong)').each(function(){
	    	var txt;
	    	txt = $(this).html().replace('via Boleto BancÃ¡rio','no boleto ou PIX');
	    	$(this).html(txt);
	    });
...
$('<span class="desc-parc">'+iconePix+''+ descontoBoleto +'</span>'
	).insertAfter('.parcelas-produto .accordion .accordion-group .accordion-heading img[alt="Boleto BancÃ¡rio"]');

/*## PAGINA CHECKOUT ##*/
$('<span class="desc-parc">'+iconePix+'</span>'
	).insertAfter('.forma-conteiner img[alt="Pague com Boleto BancÃ¡rio"]');

//Video
function videoProduto(){
		if ($(".produto-video").length) {
		var urlVideo =  document.getElementById("playerVideo").src;
		$('<div id="trailer" class="row-fluid trailer"><div class="span12"><a class="titulo-categoria cor-principal"><strong class="titulo-video">VÃ¬deo e DescriÃ§Ã£o</strong></a>'+
...
$(document).ready(function() {
  ...
  /*### PAGINA LOGIN CARRINHO ###*/
	...
	$('.pagina-carrinho #login-content > div > div > div > h3'
		).html('OlÃ¡! A Amo agradece pela sua compra! Para proceder com o pagamento, primeiramente insira seu e-mail');
	});
